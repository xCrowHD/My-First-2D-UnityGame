...
public class EnemyAIStateAttack1 : StateMachineBehaviour
{
    EnemyGM enemyGM;
    ...
    public override void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            Attack1NoPath(attackRange, CloseRange, AttackParameterName, enemyGM);
            
        }
        if(PathON == true && Time.time >= nextAttack)
        {
            Attack1WithPath(attackRange, CloseRange, AttackParameterName, enemyGM);
    ...
    private void Attack1NoPath(float attackRange, float CloseRange, string AttackParameterName, EnemyGM enemyGM)
    {
    ...
    private void Attack1WithPath(float attackRange, float CloseRange, string AttackParameterName, EnemyGM enemyGM)
    {
        if (RangeCheck(attackRange, enemyGM.playerPosition.position, enemyGM.transform.position)  && TooCloseRange(CloseRange, enemyGM.playerPosition.position, enemyGM.transform.position))
        {
